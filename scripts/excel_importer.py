#!/usr/bin/env python3
"""
Excel to PostgreSQL Import CLI - Dual Mode (Consolidated)

This is the consolidated CLI that supports both direct database access and API mode.
Previous versions (excel_importer_legacy.py, excel_importer_cli.py) have been merged.

MODES:
1. Direct mode (default): Direct database import using services
2. API mode: Makes HTTP requests to FastAPI backend (use --api-url flag)
...
HISTORY:
    - Original: excel_importer.py (monolithic, 1,056 lines, direct-only)
    - Refactored: excel_importer_cli.py (service-based, dual-mode)
    - Consolidated: 2025-01 - Merged both versions into this file
"""
